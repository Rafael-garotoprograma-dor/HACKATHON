...
export async function snapshot(db:DB,u:User){
 ...
 const transfers=(await db.query('SELECT t.*,origin.name AS from_name,destination.name AS to_name FROM transfers t JOIN classes origin ON origin.id=t.from_class JOIN classes destination ON destination.id=t.to_class ORDER BY t.created_at DESC')).rows.filter(t=>u.role==='master'||t.student_id===u.id||u.role==='professor'&&allClasses.some(c=>c.id===t.from_class&&c.professor_id===u.id));
