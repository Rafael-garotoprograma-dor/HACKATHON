...
export async function GET(){await initialize();const {database}=await import('@/lib/db');const cfg=await config(await database());return NextResponse.json({courses:cfg.courses,demo:(process.env.SHOW_DEMO_ACCOUNTS ?? process.env.DEMO_SEED)==='true'},{headers:{'Cache-Control':'no-store'}});}
